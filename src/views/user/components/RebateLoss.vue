...
import {receiveWages, wagesRecords} from "@/api/customer.ts";
import type {WagesRecordField} from "@/types/customer.type.ts";
...
const {
  ...
  loadData,
  changePage,
} = useTable<WagesRecordField>(wagesRecords, {
  defaultSize: 10
})
...
const handleReceive = async (t: number) => {
  ...
  try {
    ...
    console.error('rebate loss in error:', error)
...
<template>
  ...
  >
    ...
    <template #column-cz="{ row }">
      <span class="text-gray-400 text-xs">{{ $n(row.cz) }}</span>
    </template>
